...
# Load STAR data and take the average of all scores, since we're at the beginning of the year and can't make reliable linear models yet. Take only reading scores, then drop the subject column.
...
d.s <- subset(d.s, subject == 'ela')
d.s <- select(d.s, -subject)
...
d <- merge(d, d.s, by.x=c('student.id'), by.y=c('StudentId'))

# Generate plots
sapply(unique(d$small.school), save_star_dci_test_plot_ela_only, data=d, star.cuts=star.cuts, test.name='Benchmark 1')

# Generate excel reports
cut_star_band <- function(r, star.cuts) {
  cs <- subset(star.cuts, grade == r['grade'] & subject == 'ela')['cut']
...
writeWorksheetToFile('./../Output/DCI and STAR Band Info - ELA STAR All Subjects.xlsx', df.by.student, 'students', clearSheets=T, styleAction=XLC$"STYLE_ACTION.NONE")
writeWorksheetToFile('./../Output/DCI and STAR Band Info - ELA STAR All Subjects.xlsx', d.all, 'percentages', clearSheets=T, styleAction=XLC$"STYLE_ACTION.NONE")
